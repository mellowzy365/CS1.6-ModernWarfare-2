...
#define VERSION "1.0"
#define AUTHOR "CeLeS, Infractem, Cypis Remade Mellowzy"
...
public plugin_precache(){
	...
	format(configfile,199,"%s/custom_weaponzoom.ini",configfile);
	if(file_exists(configfile)){
		...
		for(new i=0;i<size;i++){
			...
			if((contain(row,";")!=0) && strlen(row) && idwpn<MAX){
				...
				format(V_MODEL[idwpn], 63, "models/v_%s.mdl", model);
...
public md_init()
{
	...
	md_loadimage("gfx/sights/eot.tga")
	md_loadimage("gfx/sights/eot1.tga")
	md_loadimage("gfx/sights/rds.tga")
}
...
player_fov_changed(id, iFov)
{
	...
	else
	{
		...
		switch(weapon(id))
		{
			case 14:rdsight(id,0.42,0.36); //galil
			case 20:rdsight(id,0.424,0.36); //m249
			case 23:holosight1(id,0.407,0.33); //tmp
		}
	}
}
public holosight1(id,Float:x,Float:y){
	if(!sightacg[id]){
		//acg_drawtga(id, "gfx/sights/eot.tga", 255, 255, 255, 255, 0.5, 0.5, 1, FX_FADE, 0.15, 0.0, 0.0, -1.0, 0, 0, 9)
		md_drawimage(id, 21, 0, "gfx/sights/eot1.tga", x,y, 0, 0, 255, 255, 255, 255, 0.0, 0.0, 0.0, ALIGN_NORMAL)
		sightacg[id] = true;
	}
}

public rdsight(id,Float:x,Float:y){
	if(!sightacg[id]){
		md_drawimage(id, 21, 0, "gfx/sights/rds.tga", x,y, 0, 0, 255, 255, 255, 255, 0.0, 0.0, 0.0, ALIGN_NORMAL)
...
public shotgun_crosshair(id)
{
	...
	md_drawimage(id, 19, 0, "gfx/crosshairs/horizontal.tga", 0.548,0.5,0,0,255,255,255,155,0.0,0.0,0.0)
}
...
public PreThink(id)
{
	...
	new silenced = fm_find_ent_by_owner(-1, "weapon_m4a1", id);
	new bursted = fm_find_ent_by_owner(-1, "weapon_famas", id);
	if(pev_valid(silenced))
	{
		for(new i=0;i<idwpn;i++){
			cs_set_weapon_silen(silenced, 1)

		}
	}
	if(pev_valid(bursted))
	{
		for(new i=0;i<idwpn;i++){
			cs_set_weapon_burst(bursted, 1)
